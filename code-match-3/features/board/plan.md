...
| 6 | **Acceptance stage** — `src/__tests__/acceptance.test.ts`: add stage 8 — `buildBoard` → `renderSyncSystem` → `containerMap.size === 64` → `applyCamera` centers (assert container transform). | ✅ |
| 7 | **Verify** — `pnpm test:unit` green (96/96), `pnpm typecheck` clean, `pnpm build` succeeds, `pnpm test:e2e` (3/3, `console-clean`). No `any`; no raw BiteCS outside `ecs/world.ts`; no game logic in `src/pixi/`. | ✅ |
| 8 | **Manual** — `pnpm dev` → all 64 cells visible and centered; on window resize they stay centered and fit within `[360, 960]`. *Final human gate; automated proxies in stage 8 + `console-clean` are all green.* | ⏳ pending glance |
| 9 | **Done-when** — update `features/board/acceptance.md`; mark tasks ✅; in `Current.md` move `board` → ✅ done. | ✅ |
